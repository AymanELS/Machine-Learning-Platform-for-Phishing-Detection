...
def Email_External_Spamassassin_Feature(email, list_features, list_time):
...
def Email_Header_X_Priority(priority, list_features, list_time):
    if config["Email_Header_Features"]["X_Priority"] == "True":
        start=time.time()
        if priority:
            try:
...
def HTML_inbound_count_average(soup, url_All, list_features, list_time):
    #global list_features
    if config["HTML_Features"]["inbound_count_average"] == "True":
        start=time.time()
        inbound_count = 0
        if soup:
            try:
                    ...
                    for tag in tags:
                        ...
                        if src_address != None:
                            if src_address.lower().startswith(("https", "http")):
                                ...
                                inbound_count = inbound_count + 1
                list_features["inbound_count_average"]=inbound_count/len(url_All)
            except Exception as e:
                logger.debug("exception: " + str(e))
                logger.debug("exception handled")
                list_features["inbound_count_average"]=-1
        
        end=time.time()
        ex_time=end-start
        list_time["inbound_count_average"]=ex_time
...
def HTML_outbound_count_average(soup, url_All, list_features, list_time):
    #global list_features
    if config["HTML_Features"]["outbound_count_average"] == "True":
        start=time.time()
        outbound_count = 0
        if soup:
            try:
                    ...
                    for tag in tags:
                        ...
                        if src_address != None:
                            if src_address.lower().startswith(("https", "http")):
                                ...
                                if filtered_link != local_domain:
                                    outbound_count = outbound_count + 1
                list_features["outbound_count_average"]=outbound_count/len(url_All)
            except Exception as e:
                logger.debug("exception: " + str(e))
                logger.debug("exception handled")
                list_features["outbound_count_average"]=-1
        end=time.time()
        ex_time=end-start
        list_time["outbound_count_average"]=ex_time
...
def HTML_inbound_href_count_average(soup, url_All, list_features, list_time):
    #global list_features
    if config["HTML_Features"]["inbound_href_count_average"] == "True":
        start=time.time()
                    ...
                    for tag in tags:
                        ...
                        if src_address is not None:
                            if src_address.lower().startswith(("https", "http")):
                                ...
                                inbound_href_count = inbound_href_count + 1
                inbound_href_count=inbound_href_count/len(url_All)
            except Exception as e:
                logger.debug("exception: " + str(e))
                logger.debug("exception handled")
                inbound_href_count=-1
        list_features["inbound_href_count_average"]=inbound_href_count
        end=time.time()
        ex_time=end-start
...
def HTML_outbound_href_count_average(soup, url_All, list_features, list_time):
    #global list_features
    if config["HTML_Features"]["outbound_href_count_average"] == "True":
        start=time.time()
                    ...
                    for tag in tags:
                        ...
                        if src_address is not None:
                            if src_address.lower().startswith(("https", "http")):
                                ...
                                if filtered_link != local_domain:
                                    outbound_href_count = outbound_href_count + 1
                outbound_href_count=outbound_href_count/len(url_All)
            except Exception as e:
                print("exception: " + str(e))
                outbound_href_count=-1
        list_features["outbound_href_count_average"]=outbound_href_count
        end=time.time()
        ex_time=end-start
        list_time["outbound_href_count_average"]=ex_time
    ...
    if config["Network_Features"]["DNS_Info_Exists"]=="True":
        ...
        if url:
            try:
                ...
                print("Exception: {}".format(e))
                flag=-1
                print(list_features["DNS_Info_Exists"])
        else:
            flag=0
...
def extract_email_features(dataset_path, feature_list_dict, extraction_time_dict):
    data = list()
    corpus_data = Features_Support.read_corpus(dataset_path)
    ...
    try:
        ...
        print("Total number of features files: {}".format(count_feature_files))
    ...
    for filepath in data:
        ...
        print("===================")
        print(filepath)
        Features_Support.email_features(filepath, dict_features, features_output, feature_list_dict, dict_time, extraction_time_dict, corpus)
...
def Extract_Features_Emails_Training():
    ...
    print(">>>>> Cleaning >>>>")
    print("feature_list_dict_train{}".format(len(feature_list_dict_train)))
...
def Extract_Features_Emails_Testing():
    ...
    print(">>>>> Feature extraction: Testing Set >>>>> Done ")
    print(">>>>> Cleaning >>>>")
    print("feature_list_dict_test{}".format(len(feature_list_dict_test)))
...
def Extract_Features_Urls_Training():
    ...
    if config["Email or URL feature Extraction"]["extract_features_urls"] == "True":
        ...
        print("Feature extraction time is: {}s".format(time.time() - t0)) 
        print(">>>>> Feature extraction: Training Set >>>>> Done ")
        Cleaning(feature_list_dict_train)
        print(">>>>> Cleaning >>>>>> Done")
        #print("Number of bad URLs in training dataset: {}".format(len(Bad_URLs_List)))
