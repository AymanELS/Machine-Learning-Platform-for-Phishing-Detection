...
### Email FEature Extraction
        ...
        if config["Email or URL feature Extraction"]["extract_features_emails"] == "True":
            if config["Extraction"]["Training Dataset"] == "True":
                ...
                extraction_time=time.time()-start_training
                print("Feature Extraction time: {}".format(extraction_time))
                if config["Email_Body_Features"]["tfidf_emails"] == "True":
                    ...
                    tfidf_time=time.time()-extraction_time
                    print("Tfidf render time: {}".format(tfidf_time))
                    #X_train=hstack([X_train, Tfidf_train])
                    # Save tfidf model
                    joblib.dump(tfidf_vectorizer,"Data_Dump/Emails_Training/tfidf_vectorizer.pkl")
                    joblib.dump(Tfidf_train,"Data_Dump/Emails_Training/X_train_unprocessed_with_tfidf.pkl")
...
if __name__ == "__main__":
    ...
    if answer is True:
        ...
        #sys.stdout= open("Data_Dump/Results.txt",'w')
        main()
        #sys.stdout=original
        logger.debug("Done!")
    sys.stdout=original