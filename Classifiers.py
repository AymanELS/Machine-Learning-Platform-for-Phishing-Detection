...
def fit_classifier(clf, X, y, X_train_balanced=None, y_train_balanced=None):
	...
	if X_train_balanced is not None and y_train_balanced is not None:
		...
		logging.error(np.shape(X))
		logging.error(np.shape(y))
		clf.fit(X,y)
...
def Autosklearn(X,y, X_test, y_test, X_train_balanced=None, y_train_balanced=None, clf=None):
	if clf is None:
		if config["Classifiers"]["weighted"] == "True":
			...
			clf = autosklearn.classification.AutoSklearnClassifier(ensemble_size=50,ml_memory_limit=8000)
...
def Tpot(X,y, X_test, y_test, X_train_balanced=None, y_train_balanced=None, clf=None):
	...
	if clf is None:
		if config["Classifiers"]["weighted"] == "True":
			clf = TPOTClassifier(verbosity=2, max_time_mins=2, max_eval_time_mins=0.04, population_size=40, config_dict='TPOT sparse')
		else:
			clf = TPOTClassifier(verbosity=2, max_time_mins=2, max_eval_time_mins=0.04, population_size=40, config_dict='TPOT sparse')
		logger.info("TPOT >>>>>>>")
		if config["Evaluation Metrics"]["cross_validation"]=="True":
...
def RandomForest(X,y, X_test, y_test, X_train_balanced=None, y_train_balanced=None, clf=None):
	if clf is None:
		...
		if config["Evaluation Metrics"]["parameter_search"]=="True":
			...
			fit_classifier(clf, X, y, X_train_balanced, y_train_balanced)
			logging.error(np.shape(X_test))
			logging.error(np.shape(y_test))
			y_predict=clf.predict(X_test)
...
def GaussianNaiveBayes(X,y, X_test, y_test, X_train_balanced=None, y_train_balanced=None,clf=None):
	if clf is None:
		...
		if config["Evaluation Metrics"]["parameter_search"]=="True":
			...
			y_predict=clf.predict(X_test)
			eval_metrics_GNB = Evaluation_Metrics.eval_metrics(clf, X, y, y_test, y_predict)
			return eval_metrics_GNB, clf
	else:
		y_predict=clf.predict(X_test)
		eval_metrics_GNB = Evaluation_Metrics.eval_metrics(clf, X, y, y_test, y_predict)
		return eval_metrics_GNB, clf
...
def MultinomialNaiveBayes(X,y, X_test, y_test, X_train_balanced=None, y_train_balanced=None, clf=None):
	if clf is None:
		...
		if config["Evaluation Metrics"]["parameter_search"]=="True":
			...
			fit_classifier(clf, X, y, X_train_balanced, y_train_balanced)
			y_predict=clf.predict(X_test)
			eval_metrics_MNB = Evaluation_Metrics.eval_metrics(clf, X, y, y_test, y_predict)
...
def ELM(X,y, X_test, y_test, X_train_balanced=None, y_train_balanced=None, clf=None):
	if clf is None:
		...
		if config["Evaluation Metrics"]["cross_validation"]=="True":
				...
				fit_classifier(clf, X, y, X_train_balanced, y_train_balanced)
				y_predict=clf.predict(X_test)
				eval_metrics_ELM = Evaluation_Metrics.eval_metrics(clf, X, y, y_test, y_predict)
...
def KMeans(X,y, X_test, y_test, X_train_balanced=None, y_train_balanced=None, clf=None):
	if clf is None:
		...
		y_predict=clf.predict(X_test)
		eval_metrics_Kmeans = Evaluation_Metrics.eval_metrics(clf, X, y, y_test, y_predict)
		return eval_metrics_Kmeans, clf
...
def Bagging(X,y, X_test, y_test, X_train_balanced=None, y_train_balanced=None, clf=None):
	if clf is None:
		...
		clf=BaggingClassifier(base_estimator=base_classifier, n_estimators=90, max_samples=1.0, max_features=1.0,
			bootstrap=False, bootstrap_features=True, oob_score=False, warm_start=False, n_jobs=-1, random_state=None,
			verbose=0)
		...
		y_predict=clf.predict(X_test)
		eval_metrics_Begging = Evaluation_Metrics.eval_metrics(clf, X, y, y_test, y_predict)
		return eval_metrics_Begging, clf
...
def classifiers(X,y, X_test, y_test, X_train_balanced=None, y_train_balanced=None):
	...
	if config["Classifiers"]["Tpot"] == "True":
		...
		if config["Classification"]["save model"] == "True" and model is not None:
			joblib.dump(model, "Data_Dump/Models/model_tpot.pkl")
		summary.write("Tpot\n")
